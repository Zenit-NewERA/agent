 ...
 *    $Revision: 2.9 $
 *
 *    $Id: MemPosition.h,v 2.9 2004/08/29 14:07:21 anton Exp $
 *
 ************************************************************************/
/***************************************************************************
 *                                                                         *
...
public:
  ...
  { if (!pos_valid()) my_error ("can't get_abs_pos %d",int(type));  return gpos; }
...
public:
  ...
  { if (!vel_valid()) my_error ("can't get_abs_vel %d",int(type)); return gvel; }
  ...
  //AI: for hetro players
  void set_motion_decay(float md){motion_decay=md;}
...
public:
  ...
  /////
  void set_pointing(AngleDeg ang, Time time);
  void set_tackling(Time time);
  void reset_pointing();
  void reset_tackling();
  inline float point_conf() {return ( (pointconf >= min_valid_conf) ? pointconf : 0 );}
  inline bool is_pointing() {return ( (pointconf >= min_valid_conf) ? true : false );}
  inline Time get_point_time(){return pointtime;}
  inline float tackle_conf() {return ( (tackleconf >= min_valid_conf) ? tackleconf : 0 );}
  inline bool is_tackling() {return tackling;}
  void update_tackle(Time time);
  void update_pointing(Time time);
  ////
...
public:
  ...
  ////
  void SeePlayerPointing(char side, Unum num, AngleDeg ang, Time time);
  void SeePlayerTackling(char side, Unum num, Time time);
  ////
  ...
  { return (GetBall()->catchable() && InOwnPenaltyArea() && 
	    CP_goalie && (PlayMode == PM_Play_On||PlayMode==PM_Their_PenaltyTaken))             ? TRUE : FALSE; }  
  inline float    BallKickRate       () { return GetBall()->get_kick_rate(CurrentTime);   }

  /* Access shortcuts -- Players */
  float    PlayerDistance        (char s, Unum n);
  AngleDeg PlayerAngleFromBody   (char s, Unum n);
  AngleDeg PlayerAngleFromNeck   (char s, Unum n);
  Vector   PlayerAbsolutePosition(char s, Unum n);
  Vector   PlayerRelativeToBodyPosition(char s, Unum n);
  Vector   PlayerRelativeToNeckPosition(char s, Unum n);
  float    PlayerX               (char s, Unum n);
  float    PlayerY               (char s, Unum n);
  
  float    PlayerPositionValid          (char s, Unum n);

  float    PlayerSpeed           (char s, Unum n);
  AngleDeg PlayerRelativeToBodyHeading (char s, Unum n);
  AngleDeg PlayerRelativeToNeckHeading (char s, Unum n);
  AngleDeg PlayerAbsoluteHeading (char s, Unum n);
  Vector   PlayerAbsoluteVelocity(char s, Unum n);
  Vector   PlayerRelativeToBodyVelocity(char s, Unum n);
  Vector   PlayerRelativeToNeckVelocity(char s, Unum n);
  
  float    PlayerVelocityValid          (char s, Unum n);
  
  AngleDeg PlayerRelativeToBodyBodyAngle  (char s, Unum n) ;
  AngleDeg PlayerRelativeToNeckBodyAngle  (char s, Unum n);
  AngleDeg PlayerRelativeToBodyNeckAngle  (char s, Unum n);
  AngleDeg PlayerRelativeToNeckNeckAngle  (char s, Unum n);
  AngleDeg PlayerRelativeNeckAngle (char s, Unum n);
  AngleDeg PlayerAbsoluteBodyAngle    (char s, Unum n);
  bool PlayerTackling(char s,Unum n);
  AngleDeg PlayerAbsoluteNeckAngle    (char s, Unum n) ;
  ...
  inline bool     TeammateTackling             (Unum n) { return PlayerTackling             (MySide,n); }
  ...
  inline bool     OpponentTackling             (Unum n) { return PlayerTackling             (TheirSide,n); }
  ...
  /* predicted positions */
  Vector BallEndPosition(int* num_cyc=0);//add by AI
  ...
  Vector PlayerPredictedPosition(char side, Unum num, int steps = 1, Vector dash_per=Vector(0,0));
  ...
  { return ((BallPredictedPosition(1, kick_power, kick_dir) - MyPredictedPosition()).mod() <= 
	    GetMyPlayerSize() + SP_ball_size + buffer)  ? TRUE : FALSE;  } 
  ...
  float GetTackleProb(Vector ball,Vector m,AngleDeg ba); //add by AI
  float GetPlayerTackleProb(char side,Unum num,Vector ball,bool print_log=true);//add by AI
  float GetTeammateTackleProb(Unum num,Vector ball,bool print_log=true){return GetPlayerTackleProb(MySide,num,ball,print_log);}
  float GetOpponentTackleProb(Unum num,Vector ball,bool print_log=true){return GetPlayerTackleProb(TheirSide,num,ball,print_log);}
  ...
  float ClosestOpponentToBallDistance();
  ...
  Pside GetBallSide(){if(!BallPositionValid()) my_error("not know ball pos"); return (BallY()>0?PS_Right:PS_Left);}
  ...
  float my_offside_conf; //add by AI
  Unum my_offside_opp; //add by AI
  float their_offside_line; /* Behind which other team is offside */
  float their_offside_conf;//add by AI
  Unum  their_offside_tm;//add by AI
  float their_offside_without_me;//add by AI
  ...
  float GetTheirOffsideWithoutMe()const{return their_offside_without_me;}
  ...
  float CongestionWithOnlyOpponents(Vector pos,Bool consider_me = FALSE);
  float TeammateCongestion(Unum teammate, Bool consider_me = TRUE);
  float TeammateCongestionWithOnlyOpponents(Unum teammate, Bool consider_me = TRUE);
  inline float MyCongestion() { return TeammateCongestion(MyNumber,FALSE); }
  inline float MyCongestionWithOnlyOpponents() {
    return TeammateCongestionWithOnlyOpponents(MyNumber,FALSE);      
  }
  ...
  //////////////////////////////////////////
  float AttackCheckPointsX[4];

  float AttackZoneYOffset;
  float AttackZoneWidth;
  //HETRO STUFF - add by AI////////////////////////////////////////
  int SetPlayerType(char side,Unum num,int type);//return old value
  int SetTeammatePlayerType(Unum num,int type){ return SetPlayerType(MySide,num,type);}
  int SetOpponentPlayerType(Unum num,int type){ return SetPlayerType(TheirSide,num,type);}
	
  int GetPlayerType(char side,Unum num)const;
  int GetTeammatePlayerType(Unum num,int type)const{ return GetPlayerType(MySide,num);}
  int GetOpponentPlayerType(Unum num,int type)const{ return GetPlayerType(TheirSide,num);}

  float GetPlayerPlayerSpeedMax(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetPlayerSpeedMax();}
  float GetPlayerStaminaIncMax(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetStaminaIncMax();}
  float GetPlayerPlayerDecay(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetPlayerDecay();}
  float GetPlayerInertiaMoment(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetInertiaMoment();}
  float GetPlayerDashPowerRate(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetDashPowerRate();}
  float GetPlayerPlayerSize(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetPlayerSize();}
  float GetPlayerKickableMargin(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetKickableMargin();}
  float GetPlayerKickRand(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetKickRand();}
  float GetPlayerExtraStamina(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetExtraStamina();}
  float GetPlayerEffortMax(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetEffortMax();}
  float GetPlayerEffortMin(char side,Unum num){return SP_hetro_player_type[GetPlayerType(side,num)].GetEffortMin();}	
  float GetPlayerKickableArea(char side,Unum num){return GetPlayerKickableMargin(side,num)+SP_ball_size+GetPlayerPlayerSize(side,num);}
  float GetPlayerStaminaInc(char side, Unum num){return GetPlayerStaminaIncMax(side,num);}

  float GetTeammatePlayerSpeedMax(Unum num){return GetPlayerPlayerSpeedMax(MySide,num);}
  float GetTeammateStaminaIncMax(Unum num){return GetPlayerStaminaIncMax(MySide,num);}
  float GetTeammatePlayerDecay(Unum num){return GetPlayerPlayerDecay(MySide,num);}
  float GetTeammateInertiaMoment(Unum num){return GetPlayerInertiaMoment(MySide,num);}
  float GetTeammateDashPowerRate(Unum num){return GetPlayerDashPowerRate(MySide,num);}
  float GetTeammatePlayerSize(Unum num){return GetPlayerPlayerSize(MySide,num);}
  float GetTeammateKickableMargin(Unum num){return GetPlayerKickableMargin(MySide,num);}
  float GetTeammateKickRand(Unum num){return GetPlayerKickRand(MySide,num);}
  float GetTeammateExtraStamina(Unum num){return GetPlayerExtraStamina(MySide,num);}
  float GetTeammateEffortMax(Unum num){return GetPlayerEffortMax(MySide,num);}
  float GetTeammateEffortMin(Unum num){return GetPlayerEffortMin(MySide,num);}	
  float GetTeammateKickableArea(Unum num){return GetPlayerKickableArea(MySide,num);}
  float GetTeammateStaminaInc(Unum num){return GetPlayerStaminaInc(MySide,num);}

  float GetOpponentPlayerSpeedMax(Unum num){return GetPlayerPlayerSpeedMax(TheirSide,num);}
  float GetOpponentStaminaIncMax(Unum num){return GetPlayerStaminaIncMax(TheirSide,num);}
  float GetOpponentPlayerDecay(Unum num){return GetPlayerPlayerDecay(TheirSide,num);}
  float GetOpponentInertiaMoment(Unum num){return GetPlayerInertiaMoment(TheirSide,num);}
  float GetOpponentDashPowerRate(Unum num){return GetPlayerDashPowerRate(TheirSide,num);}
  float GetOpponentPlayerSize(Unum num){return GetPlayerPlayerSize(TheirSide,num);}
  float GetOpponentKickableMargin(Unum num){return GetPlayerKickableMargin(TheirSide,num);}
  float GetOpponentKickRand(Unum num){return GetPlayerKickRand(TheirSide,num);}
  float GetOpponentExtraStamina(Unum num){return GetPlayerExtraStamina(TheirSide,num);}
  float GetOpponentEffortMax(Unum num){return GetPlayerEffortMax(TheirSide,num);}
  float GetOpponentEffortMin(Unum num){return GetPlayerEffortMin(TheirSide,num);}	
  float GetOpponentKickableArea(Unum num){return GetPlayerKickableArea(TheirSide,num);}
  float GetOpponentStaminaInc(Unum num){return GetPlayerStaminaInc(TheirSide,num);}

  float GetWorstOpponentPlayerSpeedMax(){return SP_hetro_player_type[SP_player_types].GetPlayerSpeedMax();}
  float GetWorstOpponentStaminaIncMax(){return SP_hetro_player_type[SP_player_types].GetStaminaIncMax();}
  float GetWorstOpponentPlayerDecay(){return SP_hetro_player_type[SP_player_types].GetPlayerDecay();}
  float GetWorstOpponentInertiaMoment(){return SP_hetro_player_type[SP_player_types].GetInertiaMoment();}
  float GetWorstOpponentDashPowerRate(){return SP_hetro_player_type[SP_player_types].GetDashPowerRate();}
  float GetWorstOpponentPlayerSize(){return SP_hetro_player_type[SP_player_types].GetPlayerSize();}
  float GetWorstOpponentKickableMargin(){return SP_hetro_player_type[SP_player_types].GetKickableMargin();}
  float GetWorstOpponentKickRand(){return SP_hetro_player_type[SP_player_types].GetKickRand();}
  float GetWorstOpponentExtraStamina(){return SP_hetro_player_type[SP_player_types].GetExtraStamina();}
  float GetWorstOpponentEffortMax(){return SP_hetro_player_type[SP_player_types].GetEffortMax();}
  float GetWorstOpponentEffortMin(){return SP_hetro_player_type[SP_player_types].GetEffortMin();}	
  float GetWorstOpponentKickableArea(){return GetWorstOpponentKickableMargin()+SP_ball_size+GetWorstOpponentPlayerSize();}
  float GetWorstOpponentStaminaInc(){return GetWorstOpponentStaminaIncMax();}
  //AI-particle filter stuff//////////////////////////////////////////
  void InitParticlesAgent(AngleDeg glNeckAngle);
  int checkParticlesAgent(AngleDeg angGlobalNeck);
  bool calculateStateAgent2( Vector *posGlobal,Vector *velGlobal, AngleDeg *angGlobal);
  Vector calculatePosAgentWith2Flags( MarkerType objFlag1,MarkerType objFlag2 );
  AngleDeg calculateAngleAgentWithPos( Vector pos );

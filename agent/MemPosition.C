 ...
 *    $Revision: 2.19 $
 *
 *    $Id: MemPosition.C,v 2.19 2004/08/29 14:07:21 anton Exp $
 *
 ************************************************************************/
/***************************************************************************
 *                                                                         *
...
{
  if ( !pos_valid() || !Mem->MyConf() ) return FALSE;
  return get_dist() <= Mem->GetMyKickableArea() - buffer ? TRUE : FALSE; 
}
...
float BallObject::calc_kick_rate(float dist, float ang)
{
  return  Mem->SP_kick_power_rate *
    (1 - .25 * fabs( ang ) / 180.0 -
     .25 * (dist - Mem->SP_ball_size - Mem->GetMyPlayerSize()) / Mem->GetMyKickableMargin());
}
...
void BallObject::update(Time time)
{
  ...
  //bottom code is add by AI for set 0 vel then ball is kickable for any player
  if(Mem->PlayMode==PM_Play_On){
    Unum p=Mem->PlayerWithBall();
    if(p!=Unum_Unknown&&p!=Mem->MyNumber&&((Mem->DistanceTo(gpos)>3.0f&&
       Mem->PlayerPositionValid(p<0?Mem->TheirSide:Mem->MySide,abs(p))==Mem->BallVelocityValid())||Mem->CP_goalie)){
      Mem->LogAction3(180,"Set ball to zero, becouse player %.0f control ball",float(p));
      gvel=Vector(.0f,.0f);
    }
  }
...
void BallObject::update_seen(Time time)
{
  ...
  //BallMoving() add by AI for set 0 vel then ball is kickable for player
  static float prev_speed=0.0f;
  if ( seen && !seen_moving && estimate_valid /*&& Mem->BallMoving()*/
    ...
    float coll_dist=(epos + Mem->sight_position_correction).dist(Mem->MyPos())-(Mem->GetMyPlayerSize() + Mem->SP_ball_size);
    float my_predict_speed=
      (prev_speed+Mem->LastAction->type==CMD_dash?Mem->LastAction->power*Mem->MyEffort()*Mem->GetMyDashPowerRate():0.0f)*
      Mem->GetMyPlayerDecay();
    prev_speed=Mem->MySpeed();
    Mem->LogAction7(175,"collision check: coll_dist=%.2f; my_predict_speed=%.2f; prev_speed=%.2f; was dash=%.0f (dash=%.2f)",
		    coll_dist,my_predict_speed,prev_speed,float(Mem->LastAction->type==CMD_dash),Mem->LastAction->power);
    if (((Mem->MyConf()<1.0f||Mem->MySpeed()<0.1f)&& coll_dist<0)||
	(Mem->MyConf()==1.0f&&Mem->MySpeed()<0.1f&&my_predict_speed>=0.1f&&coll_dist<0.2f)) {
      Mem->LogAction3(175, "ball: update seen predicts a collision %.2f",
		      (epos + Mem->sight_position_correction).dist(Mem->MyPos()));
      gvel = evel * -.1;
      gvconf = 0.8;//change by AI (becouse many lost ball)
      //Mem->CP_min_valid_conf / Mem->CP_ball_conf_decay; //only valid for 2 cycles
      gvtime = time;
...
void BallObject::estimate_pos(Time time)
{
  ...
  if ( get_dist() < Mem->GetMyPlayerSize() + Mem->SP_ball_size) {
    if ( vel_valid() ){
      float r = Mem->SP_ball_size + Mem->GetMyPlayerSize();
      float d = get_dist();
...
void  PlayerObject::update(Time time)
{
  ...
  if (is_tackling()) update_tackle(time);
  if (is_pointing()) update_pointing(time);
...
void  PlayerObject::reset()
{
  ...
  hbadist = hnadist = 0;
  seen_body_ang = heard_body_ang = seen_neck_ang = heard_neck_ang = FALSE;
  pointtime=tackletime=0;
  pointconf=tackleconf=0;
  tackling=FALSE;
}
...
void PlayerObject::sanitize_times()
{
  ...
  Mem->sanitize_time(tackletime);
  Mem->sanitize_time(pointtime);
}
...
void PlayerObject::set_pointing(AngleDeg ang, Time time)
{
  pointang=ang;
  pointtime=time;
  pointconf=max_conf;
}
void PlayerObject::set_tackling(Time time)
{
  tackling=TRUE;
  tackletime=time;
  tackleconf=max_conf;
}
void PlayerObject::reset_pointing()
{
  pointang=0;
  pointconf=0;
  pointtime=0;
}
void PlayerObject::reset_tackling()
{
  tackling=FALSE;
  tackleconf=0;
  tackletime=0;
}
void PlayerObject::update_tackle(Time time)
{
  if ((time-tackletime)>Mem->SP_tackle_cycles)  { reset_tackling(); return;}
  tackleconf*=conf_decay;
}
void PlayerObject::update_pointing(Time time)
{
  if ((time-pointtime)>Mem->SP_point_to_duration)  { reset_pointing(); return;}
  pointconf*=conf_decay;
}
...
/********************************************************************************/
//Add by AI/////////////////////////////////////////////////////////////////////////
//const int PositionInfo::numParticlesPosAgent=100;
/////////////////////////////////////////////////////////////////////////
void PositionInfo::Initialize()
{
  TheirGoalieNum=OurGoalieNum=Unum_Unknown;
  ...
  AttackCheckPointsX[0]= 16.0f;
  AttackCheckPointsX[1]= 40.3f;
  AttackCheckPointsX[2]= 64.6f;
  AttackCheckPointsX[3]= 89.0f;
...
void PositionInfo::SeePlayerPointing(char side, Unum num, AngleDeg ang, Time time)
{
  PlayerObject *player;
  if ( (player = GetPlayer(side, num)) == NULL )
    player = GetNewPlayer(side, num);
  if (player!=NULL) player->set_pointing(ang,time);
  else my_error("Can't get a player to see %c %d ",side,num);
}

void PositionInfo::SeePlayerTackling(char side, Unum num, Time time)
{
  PlayerObject *player;
  if ( (player = GetPlayer(side, num)) == NULL )
    player = GetNewPlayer(side, num);
  if (player!=NULL) player->set_tackling(time);
  else my_error("Can't get a player to see %c %d ",side,num);
}
...
/*********************************************************************************/
/********************************************************************************/
   float    PositionInfo::PlayerDistance(char s, Unum n)
{
     if(s==MySide && n==MyNumber)
       return 0;
     PlayerObject* pl=GetPlayer(s,n);
     if(pl==NULL){
       my_error("PlayerDistance:wrong return of GetPlayer");
       return 200.0f;
     }else
       return pl->get_dist();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerAngleFromBody(char s, Unum n) 
  {
    if(s==MySide && n==MyNumber)
       return 0;
     PlayerObject* pl=GetPlayer(s,n);
     if(pl==NULL){
       my_error("PlayerAngleFromBody:wrong return of GetPlayer");
       return 0.0f;
     }else
       return pl->get_ang_from_body();
  }
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerAngleFromNeck   (char s, Unum n) 
{
    if(s==MySide && n==MyNumber)
       return 0;
     PlayerObject* pl=GetPlayer(s,n);
     if(pl==NULL){
       my_error("PlayerAngleFromNeck :wrong return of GetPlayer");
       return 0.0f;
     }else
       return pl->get_ang_from_neck();
}
//////////////////////////////////////////////////////////////////////
Vector   PositionInfo::PlayerAbsolutePosition(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
       return MyPos();
     PlayerObject* pl=GetPlayer(s,n);
     if(pl==NULL){
       my_error("PlayerAbsolutePosition :wrong return of GetPlayer");
       return Vector(1000.0f,1000.0f);
     }else
       return pl->get_abs_pos();
}
//////////////////////////////////////////////////////////////////////
Vector   PositionInfo::PlayerRelativeToBodyPosition(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
       return Vector(0,0);
     PlayerObject* pl=GetPlayer(s,n);
     if(pl==NULL){
       my_error("PlayerRelativeToBodyPosition :wrong return of GetPlayer");
       return Vector(1000.0f,1000.0f);
     }else
       return pl->get_rel_to_body_pos();
}
//////////////////////////////////////////////////////////////////////
Vector   PositionInfo::PlayerRelativeToNeckPosition(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
       return Vector(0,0);
     PlayerObject* pl=GetPlayer(s,n);
     if(pl==NULL){
       my_error("PlayerRelativeToNeckPosition :wrong return of GetPlayer");
       return Vector(1000.0f,1000.0f);
     }else
       return pl->get_rel_to_neck_pos();
}
//////////////////////////////////////////////////////////////////////
float    PositionInfo::PlayerX(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
       return MyX();
     PlayerObject* pl=GetPlayer(s,n);
     if(pl==NULL){
       my_error("PlayerX :wrong return of GetPlayer");
       return 1000.0f;
     }else
       return pl->get_x();
}
//////////////////////////////////////////////////////////////////////
float    PositionInfo::PlayerY(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
       return MyY();
     PlayerObject* pl=GetPlayer(s,n);
     if(pl==NULL){
       my_error("PlayerY :wrong return of GetPlayer");
       return 1000.0f;
     }else
       return pl->get_y();
}
...
float PositionInfo::PlayerSpeed(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
       return MySpeed();
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerSpeed :wrong return of GetPlayer");
    return 0.0f;
  }else
    return pl->get_speed();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerRelativeToBodyHeading (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
       return 0;
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeToBodyHeading :wrong return of GetPlayer");
    return 0.0f;
  }else
    return pl->get_rel_to_body_heading();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerRelativeToNeckHeading (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
       return 0;
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeToNeckHeading :wrong return of GetPlayer");
    return 0.0f;
  }else
    return pl->get_rel_to_neck_heading();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerAbsoluteHeading (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return MyDir();
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerAbsoluteHeading :wrong return of GetPlayer");
    return 0.0f;
  }else
    return pl->get_abs_heading();
}
//////////////////////////////////////////////////////////////////////
Vector   PositionInfo::PlayerAbsoluteVelocity(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return MyVel();
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerAbsoluteVelocity :wrong return of GetPlayer");
    return Vector(0,0);
  }else
    return pl->get_abs_vel();
}
//////////////////////////////////////////////////////////////////////
Vector PositionInfo::PlayerRelativeToBodyVelocity(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return Polar2Vector(MySpeed(),0);
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeToBodyVelocity :wrong return of GetPlayer");
    return Vector(0,0);
  }else
    return pl->get_rel_to_body_vel();
}
//////////////////////////////////////////////////////////////////////
Vector PositionInfo::PlayerRelativeToNeckVelocity(char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return Polar2Vector(MySpeed(),0);
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeToNeckVelocity:wrong return of GetPlayer");
    return Vector(0,0);
  }else
    return pl->get_rel_to_neck_vel();
}
//////////////////////////////////////////////////////////////////////
...
AngleDeg PositionInfo::PlayerRelativeToBodyBodyAngle  (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return 0;
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeToBodyBodyAngle :wrong return of GetPlayer");
    return 0;
  }else
    return pl->get_rel_to_body_body_ang();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerRelativeToNeckBodyAngle  (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return 0;
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeToNeckBodyAngle :wrong return of GetPlayer");
    return 0;
  }else
    return pl->get_rel_to_neck_body_ang();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerRelativeToBodyNeckAngle  (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return 0;
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeToBodyNeckAnglee :wrong return of GetPlayer");
    return 0;
  }else
    return pl->get_rel_to_body_neck_ang();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerRelativeToNeckNeckAngle  (char s, Unum n)
{
  if(s==MySide && n==MyNumber)
    return 0;
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeToNeckNeckAngle :wrong return of GetPlayer");
    return 0;
  }else
    return pl->get_rel_to_neck_neck_ang();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerRelativeNeckAngle (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return 0;
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerRelativeNeckAngle :wrong return of GetPlayer");
    return 0;
  }else
    return pl->get_neck_rel_ang();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerAbsoluteBodyAngle    (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return MyBodyAng();
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerAbsoluteBodyAngle :wrong return of GetPlayer");
    return 0;
  }else
    return pl->get_abs_body_ang();
}
//////////////////////////////////////////////////////////////////////
bool PositionInfo::PlayerTackling(char s,Unum n)
{
  if(s==MySide && n==MyNumber)
    return Tackling();
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerTackling :wrong return of GetPlayer");
    return false;
  }else
    return pl->is_tackling();
}
//////////////////////////////////////////////////////////////////////
AngleDeg PositionInfo::PlayerAbsoluteNeckAngle    (char s, Unum n) 
{
  if(s==MySide && n==MyNumber)
    return MyNeckGlobalAng();
  PlayerObject* pl=GetPlayer(s,n);
  if(pl==NULL){
    my_error("PlayerAbsoluteNeckAngle :wrong return of GetPlayer");
    return false;
  }else
    return pl->get_abs_neck_ang();
}
//////////////////////////////////////////////////////////////////////
...
/*  inline AngleDeg PlayerRelativeToBodyBodyAngle  (char s, Unum n) 
  ...
  inline bool PlayerTackling(char s,Unum n)
  {return (s==MySide && n==MyNumber) ?Tackling():GetPlayer(s,n)->is_tackling();}
...
/********************************************************************************/
Vector PositionInfo::PlayerPredictedPosition(char side, Unum num, int steps, Vector dash_per)
{ 
  PlayerObject *player = GetPlayer(side, num);
  if ( player == NULL ) 
    return Vector(1000.0,1000.0);
  else 
    return player->estimate_future_pos(steps, 0, dash_per); 
}
//////////////////////////////////////////////////////////////////////
//AI: this results was reseaved from analyze grathic of tackle probability in Mathcad
float PositionInfo::GetTackleProb(Vector ball,Vector my_pos,AngleDeg my_body_ang){
  if((ball-my_pos).mod()>3.0f)
    return 0.0f;
  if(SP_tackle_dist!=2.0||SP_tackle_back_dist!=0.5||SP_tackle_width!=1.0||SP_tackle_exponent!=6.0){
    my_error("Parametres of tackle was changed! Function GetTackleProb() must be changed");
    return 0.0f;
  }
  Vector player_2_ball=(ball-my_pos).rotate(-my_body_ang);
  float td=player_2_ball.x>0?SP_tackle_dist:SP_tackle_back_dist;
  float conf=1-int_pow(fabs(player_2_ball.x)/td,SP_tackle_exponent)-int_pow(fabs(player_2_ball.y)/SP_tackle_width,SP_tackle_exponent);
  LogAction5(50,"GetTackleProb(): player_2_ball=(%.4f,%.4f); conf=%.2f",player_2_ball.x,player_2_ball.y,conf);
  return player_2_ball.x>=-0.3f&&player_2_ball.x<=1.5f/*was 1.4f*/&&fabs(player_2_ball.y)<=0.57f/* was 0.6f*/;
}
/********************************************************************************/
float PositionInfo::GetPlayerTackleProb(char side,Unum num,Vector ball,bool print_log){
  if(side==MySide&&num==MyNumber)
    return GetTackleProb(ball,MyPos(),MyBodyAng());
  if(!PlayerPositionValid(side,num)||PlayerDistanceTo(side,num,ball)>3.0f)
    return 0.0f;
  AngleDeg body_ang;
  float body_conf=PlayerBodyAngleValid(side,num),vel_conf=PlayerVelocityValid(side,num);
  //  if(body_conf==1.0f||body_conf>=vel_conf){
  //    if(!body_conf)
  //      return 0.0f;
  //    body_ang=PlayerAbsoluteBodyAngle(side,num);
  //  }else if(vel_conf)
  //    body_ang=PlayerAbsoluteVelocity(side,num).dir();
  //  else return 0.0f;
  if(body_conf==1.0f)
    body_ang=PlayerAbsoluteBodyAngle(side,num);
  else
    body_ang=(ball-PlayerAbsolutePosition(side,num)).dir();
  Vector player_2_ball=(ball-PlayerAbsolutePosition(side,num)).rotate(-body_ang);
  float td=player_2_ball.x>0?SP_tackle_dist:SP_tackle_back_dist;
  float conf=1-int_pow(fabs(player_2_ball.x)/td,SP_tackle_exponent)-int_pow(fabs(player_2_ball.y)/SP_tackle_width,SP_tackle_exponent);
  if(print_log){
    LogAction7(10,"GetPlayerTackleProb:side=%.0f; num=%.0f; pos_conf=%.2f; body_conf=%.2f; vel_conf=%.2f",
	       float(side),float(num),PlayerPositionValid(side,num),body_conf,vel_conf);
    LogAction5(50,"GetPlayerTackleProb: player_2_ball=(%.4f,%.4f); conf=%.2f",player_2_ball.x,player_2_ball.y,conf);
  }
  return player_2_ball.x>=-0.4f&&player_2_ball.x<=1.6f&&fabs(player_2_ball.y)<=0.65f;
}

/********************************************************************************/
/********************************************************************************/
Vector PositionInfo::BallEndPosition(int* cycles){//add by AI
  if(!Mem->BallPositionValid()){
    my_error("BallEndPosition: ball position not valid!");
    return Vector(0,0);
  }
  Vector ball=Mem->BallAbsolutePosition();
  if(Mem->BallVelocityValid()&&!Mem->BallMoving())
    return ball;
  int num_cyc=1000;
  Unum tm= Mem->FastestTeammateToBall();
  if(tm!=Unum_Unknown&&Mem->TeammateInterceptionAble(tm)){
    ball=Mem->TeammateInterceptionPoint(tm);
    num_cyc=Mem->TeammateInterceptionNumberCycles(tm);
  }
  Unum opp=Mem->FastestOpponentToBall();
  if(opp!=Unum_Unknown&&Mem->OpponentInterceptionAble(opp)&&num_cyc>Mem->OpponentInterceptionNumberCycles(opp)){
    ball=Mem->OpponentInterceptionPoint(opp);
    num_cyc=Mem->OpponentInterceptionNumberCycles(opp);
  }
  if(cycles!=0)
    *cycles=num_cyc;
  return ball;	
...
Bool PositionInfo::BallWillBeKickable(int steps, float dash_power, float buffer)
{
  ...
  return my_predicted_position.dist(ball_predicted_position) <= GetMyKickableArea()-buffer ? TRUE : FALSE; 
}
					       ...
					       float dash_power, float buffer)
{
  ...
  if((side==MySide&&num==OurGoalieNum)||(side==TheirSide&&num==TheirGoalieNum))
    buffer=max(buffer,SP_catch_area_l);
  ...
  for (int i=0; TRUE; i++) {
    ...
    if (fabs(targ_ang) > CP_max_go_to_point_angle_err) {
      ...
      /* dashing */
      velocity += Polar2Vector( dash_power*GetPlayerDashPowerRate(side,num), bodyang );
      LogAction6(210, "PlayerPredicatedPosition: %d at (%.1f, %.1f) spd: %.2f. dashing",
		 i, position.x, position.y, velocity.mod());
    }
    
    if ( velocity.mod() > GetPlayerPlayerSpeedMax(side,num) )
      velocity *= ( GetPlayerPlayerSpeedMax(side,num)/velocity.mod() );
    
    position += velocity;
    velocity *= GetPlayerPlayerDecay(side,num);
  }
}
...
PlayerObject *PositionInfo::GetNewPlayer(char side, Unum num)
{
  ...
  player->side = side;
  player->unum = num;
  player->set_motion_decay(GetPlayerPlayerDecay(side,num));
  player->set_max_speed(GetPlayerPlayerSpeedMax(side,num));
...
Bool PositionInfo::BallKickableForPlayer(char s, Unum n, float buffer)
{
  ...
  return ((pos - BallAbsolutePosition()).mod() <
	  GetPlayerKickableArea(s,n) - buffer) ? TRUE : FALSE;
}
...
void PositionInfo::update()
{
  ...
  if (CP_save_action_log_level >= 175) {
    ...
    if (!Mem->MyConf())
      ...
      sprintf(outstring, "My Pos: (%.2f, %.2f)\tangle: %.2f\trel_neck: %.2f\tvel: (%.2f %.2f)\tconf: %.2f\tstamina: %.2f\tattention: %.0f",
	      MyX(), MyY(), MyBodyAng(), MyNeckRelAng(), MySpeed(), MyDir(), MyConf(), MyStamina(),(float)attention_to_player);
  ...
  //  //added by serg
  eye.UpdateConfs(CurrentTime,NewSight);
...
void PositionInfo::update_self_seen(Time time)
{
  ...
  if ( SeenLine != SL_No_Line && ClosestMarker != No_Marker ){
    ...
    //    if(ViewWidth!=VW_Narrow||MarkerDistance(ClosestMarker)>40.0f){
    SetMyPos( Marker[ClosestMarker].get_my_pos(MyNeckGlobalAng()), time );
    //      PlayerInfo::InitParticlesAgent(pos);
    //      goto reset;
    //    }
    
    //    updateParticleAgent(MyVel(),false);
    //    int iNrLeft=checkParticlesAgent(MyNeckGlobalAng());
    //
    //    GlobalVel=MyVel();
    //    GlobalAng=MyNeckGlobalAng();
    //
    //    if(iNrLeft==0){
    ////      LogAction3(10,"check filter in %s",(char*)__FUNCTION__);
    //      InitParticlesAgent(MyNeckGlobalAng());
    //      iNrLeft=checkParticlesAgent(MyNeckGlobalAng());
    //      if(iNrLeft==0){
    //	    calculateStateAgent2(&pos,&GlobalVel,&GlobalAng);
    //        SetMyPos(pos,time);
    //        PlayerInfo::InitParticlesAgent(pos);
    ////        LogAction4(10,"Go away in update_self_seen (%f,%f)",particlePos.x,particlePos.y);
    //        goto reset;
    //      }
    //    }
    //    SetMyPos(averageParticles(particlesPosAgent,iNrLeft),time);
    //    resampleParticleInfo(iNrLeft);
    ////    LogAction4(10,"Get res in update_self_seen (%f,%f)",particlePos.x,particlePos.y);
    //
    //    GlobalAng=calculateAngleAgentWithPos(MyPos());
    //    if(GlobalAng==-777.0)
    //      GlobalAng=MyNeckGlobalAng();
    //    GlobalVel=Global2RelativeToMyNeck(GlobalVel).rotate(GlobalAng);
    //
  }
  //
  //reset:
  //set new neck angle and vel
  //  LogAction4(10,"Old rel neck:%.4f; new rel neck: %.4f",MyNeckRelAng(),GetNormalizeAngleDeg(GlobalAng-MyBodyAng()));
  //  SetMyNeckRelAng(GetNormalizeAngleDeg(GlobalAng-MyBodyAng()));
  //  SetMyVel(GlobalVel,time);
  //erace old values
  for (int i=0; i<num_seen_markers; i++)
...
//HETRO PLAYERS - add by AI/////////////////////////////////////////////////////////////////////////////////
int PositionInfo::SetPlayerType(char side,Unum num,int type){
  if(side==MySide&&num==MyNumber){
    return SetMyPlayerType(type);
  }
  int add=0;
  if(side==TheirSide)
    add=SP_team_size;
    ...
    if(side!=MySide){
      my_error("SetPlayerType: wrong value of side %c",side);
      return -1;
    }
  if(num<1||num>SP_team_size){
    my_error("SetPlayerType: wrong value of num %d",num);
    return -1;
  }
  if(type<0||type>Mem->SP_player_types){
    my_error("SetPlayerType: type %d is wrong",type);
    return players_type[add+num-1];
  }
  int temp=players_type[add+num-1];
  players_type[add+num-1]=type;
  PlayerObject* player=GetPlayer(side,num);
  if(player!=0){
    player->set_motion_decay(GetPlayerPlayerDecay(side,num));
    player->set_max_speed(GetPlayerPlayerSpeedMax(side,num));
...
//----------------------------------------------------------------------
int PositionInfo::GetPlayerType(char side,Unum num)const{
  if(side==MySide&&num==MyNumber){
    return GetMyPlayerType();
  }
  int add=0;
  if(side==TheirSide)
    add=SP_team_size;
  if(num==Unum_Teamless||num==Unum_Unknown)
    return DEFAULT_HETRO_PLAYER_TYPE_NUMBER;
  if(num<1||num>SP_team_size){
    my_error("GetPlayerType: wrong value of num %d",num);
    return -1;
  }
  return players_type[add+num-1];	
}
...
Vector PositionInfo::ClosestTeamlessPlayerPosition()
{
  ...
  float tmp,closest_dist = 100*SP_pitch_length;
  Vector position;
  for (int i=0; i<num_teamless_players; i++){
...
/********************************************************************************/
float PositionInfo::ClosestOpponentToBallDistance()
{
  if ( !BallPositionValid() ) my_error("can't do closest if ball unknown");
  Unum opp=ClosestOpponentTo(BallAbsolutePosition());
  return opp==Unum_Unknown?777.0:BallAbsolutePosition().dist(OpponentAbsolutePosition(opp));
}
...
Unum PositionInfo::FurthestBackTeammate(Bool IncludeUs, Bool IncludeGoalie)
{
  ...
  float first_x =  100*SP_pitch_length;
  //  Pnum tmp;
  for (Unum num=1; num <= SP_team_size; num++) {
    if (!TeammatePositionValid(num))
      continue;
...
/*********************************************************************************/
float PositionInfo::CongestionWithOnlyOpponents(Vector pos,Bool consider_me) {
  ...
  for (i=0; i<num_players; i++)
    /* Don't want to count a player in its own congestion measure */
    if( Player[i].side == MySide ) continue;
  if ( Player[i].pos_valid() && (player_pos = Player[i].get_abs_pos()) != pos )
    congestion += 1/player_pos.dist2(pos);
...
/*********************************************************************************/
float PositionInfo::TeammateCongestionWithOnlyOpponents(Unum teammate, Bool consider_me)
{
  if ( !TeammatePositionValid(teammate) ) my_error("unknown teammate congestion");
  return CongestionWithOnlyOpponents(TeammateAbsolutePosition(teammate),consider_me);
}
/*********************************************************************************/
...
//Particles stuff///////////////////////////////////////////////////////
void PositionInfo::InitParticlesAgent(AngleDeg glNeckAngle){
  double f,fMinRadius,fMaxRadius;
  AngleDeg ang;
  if(ClosestMarker==No_Marker){
    my_error("PlayerInfo::%s: no flag in last see massege",(char*)__FUNCTION__);
    return;
  }
  float dist=MarkerDistance(ClosestMarker);
  getMinMaxDistQuantizeValue((double)dist, &fMinRadius, &fMaxRadius,SP_land_qstep, 0.1 ) ;
  AngleDeg angFlag=MarkerAngleFromNeck(ClosestMarker)+180+glNeckAngle;
  for(int i=0;i<numParticlesPosAgent;i++){
    f=fMinRadius+drand48()*(fMaxRadius-fMinRadius);
    ang=GetNormalizeAngleDeg(angFlag-1.0+2*drand48());
    particlesPosAgent[i]=MarkerPosition(ClosestMarker)+Polar2Vector(f,ang);
  }
}
/////////////////////////////////////////////////////////
int PositionInfo::checkParticlesAgent(AngleDeg angGlobalNeck){
  double fMinRadius,fMaxRadius,fInput,fDist;
  AngleDeg ang;
  MarkerType m;
  int iNrLeft=0,iLength=numParticlesPosAgent;

  for(int i=0;i<num_seen_markers;i++)
    Marker[SeenMarker[i]].sanitize_times();

  for (int i=0; i<num_seen_markers; i++){
    m=SeenMarker[i];
    iNrLeft=0;
    fInput=(double)MarkerDistance(m);

    getMinMaxDistQuantizeValue(fInput, &fMinRadius, &fMaxRadius,SP_land_qstep, 0.1 ) ;
    for(int j=0;j<iLength;j++){
      fDist=particlesPosAgent[j].dist(MarkerPosition(m));
      ang=(MarkerPosition(m)-particlesPosAgent[j]).dir();
      ang=ang-(MarkerAngleFromNeck(m)+angGlobalNeck);
      if(fDist>fMinRadius&&fDist<fMaxRadius&&fabs(GetNormalizeAngleDeg(ang))<=1.0)
        particlesPosAgent[iNrLeft++]=particlesPosAgent[j];
    }
    iLength=iNrLeft;
  }
  //  Mem->LogAction5(10,"checkParticleAgent:: num_seeen_markers=%.0f; iNrLeft=%.0f; closest mark dist:%f",float(num_seen_markers),float(iNrLeft),
  //      MarkerDistance(ClosestMarker));
  return iNrLeft;
}
//////////////////////////////////////////////////////////////////////////
bool PositionInfo::calculateStateAgent2( Vector *posGlobal,
					 Vector *velGlobal, AngleDeg *angGlobal)
{
  double      x=0.0, y=0.0, dMinRadius1, dMaxRadius1, dMinRadius2, dMaxRadius2;
  double      dTotalVar = -777.0, dVar, K;
  int         iIndex1, iIndex2;
  Vector pos;
   
  // for all flags that were perceived in last see message
  for(iIndex1=0;iIndex1<num_seen_markers;iIndex1++)
    {
      // calculate global position with all other flags using two flags
      for(iIndex2 = iIndex1+1;iIndex2<num_seen_markers;iIndex2++)
	{
	  // calculate the position using the two flags
	  pos = calculatePosAgentWith2Flags( SeenMarker[iIndex1],SeenMarker[iIndex2] );
	  ...
	  // intensive
	  getMinMaxDistQuantizeValue(MarkerDistance(SeenMarker[iIndex1]),
				     &dMinRadius1, &dMaxRadius1, SP_land_qstep, 0.1 )  ;
	  getMinMaxDistQuantizeValue(MarkerDistance(SeenMarker[iIndex2]),
				     &dMinRadius2, &dMaxRadius2, SP_land_qstep, 0.1 )  ;
  ...
  *velGlobal = Global2RelativeToMyNeck(MyVel()).rotate(*angGlobal);
 
  return true;
}
//////////////////////////////////////////////////////////////////////
Vector PositionInfo::calculatePosAgentWith2Flags( MarkerType objFlag1,MarkerType objFlag2 )
{
  double      xA, yA, xB, yB, rA, rB;
  AngleDeg      aA, aB;
 
  // get all information of the two flags
  xA = MarkerPosition  ( objFlag1 ).x;
  yA = MarkerPosition  ( objFlag1 ).y;
  rA = MarkerDistance  ( objFlag1 );
  aA = MarkerAngleFromNeck   ( objFlag1 );
  xB = MarkerPosition  ( objFlag2 ).x;
  yB = MarkerPosition  ( objFlag2 ).y;
  rB = MarkerDistance ( objFlag2 );
  aB = MarkerAngleFromNeck ( objFlag2 );
...
////////////////////////////////////////////////////////////////////// 
AngleDeg PositionInfo::calculateAngleAgentWithPos( Vector pos )
{
  int    iNrFlags = 0;
  ...
  for(int i=0;i<num_seen_markers;i++)
    {
      xA = MarkerPosition(SeenMarker[i]).x;
      yA = MarkerPosition(SeenMarker[i]).y;
      aA = MarkerAngleFromNeck(SeenMarker[i] );

      // calculate global direction between flag and agent
      // calculate global neck angle by subtracting relative angle to flag
      dAngleNow = ATan2( yA - pos.y, xA - pos.x );
      dAngleNow = GetNormalizeAngleDeg( dAngleNow - aA );
      ...
      dCosX += Rad2Deg(Cos( dAngleNow ));
      dSinX += Rad2Deg(Sin( dAngleNow ));
      iNrFlags++;
  ...
  return GetNormalizeAngleDeg( ATan2( dSinX, dCosX  ) )  ;
}
